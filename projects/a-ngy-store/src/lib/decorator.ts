...
import { DIContainer } from "./a-ngy-store.module";
...
export function connectTo<T, R = any>(settings?: ((store: Store<T>) => Observable<R>) | ConnectToSettings<T, R>) {
  ...
  function getSource(selector: (((store: Store<T>) => Observable<R>))): Observable<any> {
    ...
    const store = $store || ($store = DIContainer.get(Store));
  ...
  return function (target: any) {
    ...
    target.prototype[typeof settings === "object" && settings.setup ? settings.setup : "ngOnInit"] = function () {
    ...
    target.prototype[typeof settings === "object" && settings.teardown ? settings.teardown : "ngOnDestroy"] = function () {
